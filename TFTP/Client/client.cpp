...
void initializeTransfer(uint16_t opcode, char* filename, int socket_fd, sockaddr_in serverAddr, int file_fd, uint16_t &block_num){
    switch (opcode) {
        case OP_WRQ:
    ...
    int bytesSent = sendMessage(socket_fd, serverAddr, (void*) &message, sizeof(OP_RRQ)+strlen(filename)+strlen(mode));
}
